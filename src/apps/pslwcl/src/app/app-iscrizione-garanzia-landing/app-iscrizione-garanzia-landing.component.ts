...
@Component({
  selector: 'app-iscrizione-garanzia-landing',
  templateUrl: './app-iscrizione-garanzia-landing.component.html'
})
export class AppIscrizioneGaranziaLandingComponent implements OnInit {
  constructor(
  ...
  controllaUtente() {
    ...
    this.utilitiesService.setLinkLand('/iscrizione-garanzia');
    // Non ho l'utente e sono in sviluppo/test, allora pagina di login
    if (ConfigService.useAutenticationPage()) {
      this.logService.log("iscrizione landing---------------->" + location.href);
      this.logService.log("iscrizione landing---------------->" + new URL(location.href).searchParams.get("param"));
